...
function mapApiSubscription(s: any): Subscription {
  ...
  return {
    id: s.id,
    userId: s.userId ?? 0,
    user: s.userName || ('User #' + (s.userId ?? '?')),
    email: s.userEmail || '',
    plan: planName === 'premium' || planName === 'pro' || planName === 'free' ? (planName === 'premium' ? 'elite' : planName) : 'pro',
    ...
    status: status === 'active' || status === 'trial' ? 'active' : (status === 'cancelled' || status === 'canceled' ? 'cancelled' : status)
  };
}
...
export class BoSubscriptionManagementComponent implements OnInit {
  ...
  loadFromApi(): void {
    this.subscriptionService.listSubscriptions().subscribe({
      ...
      error: (err) => console.warn('[BO subscription-management] failed to load:', err)
    });
  }
