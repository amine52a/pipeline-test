...
public class CertificationController {
    ...
    public String testEmail(@PathVariable Integer certificationId) {
        try {
            ...
            if (cert.getUserEmail() == null) {
                return "❌ User email not found. User ID: " + cert.getUserId();
            }

            emailService.sendCertificationEmail(cert, cert.getUserEmail());
            return "✅ Email sent to: " + cert.getUserEmail();
    ...
    public Certification addCertification(@RequestBody Certification certification) {
        try {
            ...
            // 2️⃣ L'email a déjà été envoyé par le service si l'utilisateur a été résolu
            if (savedCert.getUserEmail() == null) {
                System.err.println("⚠️ Warning: Could not resolve user email for user_id=" + savedCert.getUserId());
            }
