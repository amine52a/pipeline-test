...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    const dbRole = normalizedRole.toUpperCase(); // schema uses CLIENT/FREELANCER/ADMIN
    const skillsValue = Array.isArray(skills) ? skills.join(',') : skills;
    const [result] = await pool.query(
      'INSERT INTO users (first_name, last_name, email, password, role, location, skills, bio) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [firstName, lastName, email, passwordHash, dbRole, location, skillsValue, bio]
    );
