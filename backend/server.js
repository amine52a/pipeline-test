...
// JWT shared secret — must match `matchy.jwt.secret` in every Spring service
const JWT_SECRET = process.env.JWT_SECRET || 'dev-shared-secret-change-me-please-please-please-32bytes';
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    // Accept either fullName OR firstName+lastName from her register UI.
    ...
    if (!fullName || !email || !password || !role) {
      return res.status(400).json({ error: 'fullName (or firstName+lastName), email, password and role are required' });
    }
    ...
    const dbRole = normalizedRole.toUpperCase(); // existing schema uses CLIENT/FREELANCER/ADMIN
    const skillsValue = Array.isArray(skills) ? skills.join(',') : skills;
    const [result] = await pool.query(
      'INSERT INTO users (first_name, last_name, firstName, lastName, email, password, role, location, skills, bio) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [firstName, lastName, firstName, lastName, email, passwordHash, dbRole, location, skillsValue, bio]
    );
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    const row = rows[0];
    const ok = await bcrypt.compare(password, row.password || row.password_hash || '');
    if (!ok) return res.status(401).json({ error: 'Invalid credentials' });
    if (['SUSPENDED', 'INACTIVE', 'BANNED'].includes(String(row.status || '').toUpperCase())) {
...
// Current user (rehydrate session)
...
// Public list of all users (used by other microservices / backoffice dropdowns)
...
// Public lookup of a user by id (used by other microservices to fetch email/name)
...
// Get ALL notifications (admin / backoffice)
